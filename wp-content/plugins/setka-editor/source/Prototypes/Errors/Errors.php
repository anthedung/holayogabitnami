    ...
    public function allAsArray($code = true, $message = true, $messageHTML = true, $data = true)
    {
        ...
        if($this->hasErrors()) {
            foreach($this->errors as $error) {
                ...
                if($data) {
                    $dataValue = $error->getData();
                    if(is_a($dataValue, \Exception::class)) {
                        /**
                         * @var $dataValue \Exception
                         */
                        $singleError['data'] = array(
                            'exceptionName' => get_class($dataValue),
                            'code' => $dataValue->getCode(),
                            'message' => $dataValue->getMessage(),
                        );
                    } else {
                        $singleError['data'] = $dataValue;
                    }
                }
                $errors[] = $singleError;
            }
