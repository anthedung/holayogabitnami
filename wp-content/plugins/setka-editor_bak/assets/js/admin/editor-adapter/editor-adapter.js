...
module.exports = Backbone.View.extend({
    ...
    // we store here <link> tags with Type Kits
    views: {},
    ...
    onTypeKitIdChanged: function() {
        ...
        if(_.isString(id) && '' !== id) {
            if(_.isUndefined(this.views[id])) {
                this.views[id] = new adapter.view.TypeKit({
...
module.exports = function( text ) {
    ...
    text = text.replace( /(<(?:div|th|td|form|fieldset|dd)[^>]*>)(.*?)<\/p>/g, function( a, b, c ) {
        ...
        // This part with return state was refactored because WordPress VIP coding standards
        // does not allow merging strings with string contains html tags.
        // Original it was b + '<p>' + c + '</p>'.
        var pContainer = document.createElement('p');
        pContainer.innerHTML = c;
        return b + pContainer.outerHTML;
    });
...
module.exports = Backbone.View.extend({
    ...
    _preparePlainContent: function(content) {
        ...
        for(var i = 0; i < div.childNodes.length; i++) {
            ...
            if(node.nodeType === 3) {
                ...
                if(wrapped.indexOf('<p>') === -1) {
                    var pContainer = document.createElement('p');
                    pContainer.innerHTML = wrapped;
                    wrapped = pContainer.outerHTML;
                }
        ...
        if( needWrapper ) {
            var stkPostContainer = document.createElement('div');
            stkPostContainer.className = 'stk-post';
            stkPostContainer.innerHTML = newContent;
            newContent = stkPostContainer.outerHTML;
        }
...
module.exports = Backbone.View.extend({
    ...
    createUtils: function() {
        ...
        this.utils.typeKitManager = new adapter.utils.TypeKitManager({
            model: this.model
        });
    },
...
module.exports = Backbone.View.extend({
    ...
    el: '#wp-kit-notice-' + translations.names.css + '-' +translations.names.css + '-cant-find-resources',
