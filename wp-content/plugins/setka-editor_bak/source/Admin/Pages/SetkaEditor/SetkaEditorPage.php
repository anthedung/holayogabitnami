...
class SetkaEditorPage extends VirtualMenuPage
{
    ...
    public function fixMenu()
    {
        ...
        if(isset($submenu[$pageName][0][0])) {
            $submenu[$pageName][0][0] = $this->getVirtualPage()->getMenuTitle(); // WPCS: override ok.
        }

        if(isset($submenu[$pageName][2][2]) && $submenu[$pageName][2][2] === $pageName . '-files') {
            unset($submenu[$pageName][2]);
        }

        if(isset($submenu[$pageName][3][2]) && $submenu[$pageName][3][2] === $pageName . '-upgrade') {
            $submenu[$pageName][3][2] = PluginConfig::getUpgradeUrl(); // WPCS: override ok.
        }

        if(isset($submenu[$pageName][4][2]) && $submenu[$pageName][4][2] === $pageName . '-amp') {
            unset($submenu[$pageName][4]); // WPCS: override ok.
        }
    }
